// ============================================
// SIEC Admin Dashboard - Complete Clean Version
// ============================================
...
function generateQrWithLogo(canvasId, text, size) {
    ...
    try {
        var qr = new QRLib(text, 'H', qrSize, 2);
        qr.draw(canvas);
        overlayLogoOnCanvas(canvas, qrSize);
    } catch(e) {
        drawQrPlaceholder(canvas, qrSize);
    }
}

function generateQrCode(canvasId, text, size) {
    generateQrWithLogo(canvasId, text, size);
}

function overlayLogoOnCanvas(canvas, size) {
    var ctx = canvas.getContext('2d');
    var logo = new Image();
    logo.crossOrigin = 'anonymous';
    logo.onload = function() {
        var ls = Math.round(size * 0.22);
        var cx = size / 2;
        var cy = size / 2;
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(cx, cy, ls / 2 + 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.save();
        ctx.beginPath();
        ...
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(cx, cy, ls / 2 + 1, 0, Math.PI * 2);
        ctx.stroke();
    };
    logo.onerror = function() {
        ...
        ctx2.beginPath();
        ctx2.arc(cx, cy, ls / 2 + 3, 0, Math.PI * 2);
        ctx2.fill();
        ctx2.fillStyle = '#2563eb';
        ctx2.font = 'bold ' + Math.round(ls * 0.35) + 'px Arial';
...
function drawQrPlaceholder(canvas, size) {
    var ctx = canvas.getContext('2d');
    ctx.fillStyle = '#ffffff';
    ...
    ctx.font = Math.round(size * 0.1) + 'px Arial';
    ctx.fillStyle = '#333';
    ctx.fillText('QR', size / 2, size / 2 + size * 0.1);
}

// QR Library sederhana (tidak perlu CDN)
function QRLib(text, ecLevel, size, margin) {
    this.text = text;
    this.size = size || 150;
    this.margin = margin || 2;
    this.ecLevel = ecLevel || 'H';
}

QRLib.prototype.draw = function(canvas) {
    var self = this;
    var size = this.size;
    var ctx = canvas.getContext('2d');

    // Buat QR menggunakan Google Image API sebagai fallback
    var img = new Image();
    img.crossOrigin = 'anonymous';
    var encoded = encodeURIComponent(self.text);
    img.src = 'https://chart.googleapis.com/chart?cht=qr&chs=' + size + 'x' + size + '&chl=' + encoded + '&choe=UTF-8&chld=H|2';
    ...
    img.onload = function() {
        ...
        overlayLogoOnCanvas(canvas, size);
    };

    img.onerror = function() {
        // Fallback: coba api.qrserver.com
        var img2 = new Image();
        img2.crossOrigin = 'anonymous';
        img2.src = 'https://api.qrserver.com/v1/create-qr-code/?size=' + size + 'x' + size + '&data=' + encoded + '&ecc=H&margin=4';
        img2.onload = function() {
            ctx.drawImage(img2, 0, 0, size, size);
            overlayLogoOnCanvas(canvas, size);
        };
        img2.onerror = function() {
            drawQrPlaceholder(canvas, size);
        };
    };
};
...
function switchSection(section) {
    ...
    if (document.getElementById('adminSidebar')) document.getElementById('adminSidebar').classList.remove('active');
}
...
function handleFilePreview(input, type) {
    ...
    if (file.type === 'application/pdf') {
        ...
        if (word) {
            ...
            var n = document.getElementById(type + 'WordName');
            if (n) n.textContent = file.name;
        }
    }

    var sampleId = type === 'trans' ? 'SIEC-TR-2024-0001' : 'SIEC-TF-2024-0001';
    var url2 = window.location.origin + '/verify.html?id=' + sampleId;
    setTimeout(function() {
        generateQrWithLogo(type + 'QrCanvas', url2, 80);
    }, 800);
...
function initDrag(type) {
    ...
    el.addEventListener('mousedown', function(e) {
        ...
        sx = e.clientX; sy = e.clientY; ol = el.offsetLeft; ot = el.offsetTop; e.preventDefault();
    });
    document.addEventListener('mousemove', function(e) {
        if (!drag) return;
        moveQrEl(type, el, cont, ol+(e.clientX-sx), ot+(e.clientY-sy));
    });
    document.addEventListener('mouseup', function() {
        if (!drag) return; drag = false; el.classList.remove('dragging');
        if (type === 'toefl') savePos(el, cont);
    });
    el.addEventListener('touchstart', function(e) {
        drag = true; el.classList.add('dragging');
        var t = e.touches[0]; sx = t.clientX; sy = t.clientY; ol = el.offsetLeft; ot = el.offsetTop; e.preventDefault();
    }, { passive: false });
    document.addEventListener('touchmove', function(e) {
        if (!drag) return;
        var t = e.touches[0]; moveQrEl(type, el, cont, ol+(t.clientX-sx), ot+(t.clientY-sy)); e.preventDefault();
    }, { passive: false });
...
function moveQrEl(type, el, cont, nl, nt) {
    nl = Math.max(0, Math.min(nl, cont.offsetWidth - el.offsetWidth));
    nt = Math.max(0, Math.min(nt, cont.offsetHeight - el.offsetHeight));
    el.style.left = nl + 'px'; el.style.top = nt + 'px';
    var px = Math.round((nl/cont.offsetWidth)*100);
    var py = Math.round((nt/cont.offsetHeight)*100);
    var px2 = document.getElementById(type+'PosX'); if(px2) px2.textContent = px+'%';
    var py2 = document.getElementById(type+'PosY'); if(py2) py2.textContent = py+'%';
    var sx2 = document.getElementById(type+'QrX'); if(sx2) sx2.value = px;
    var sy2 = document.getElementById(type+'QrY'); if(sy2) sy2.value = py;
}

function savePos(el, cont) {
...
function loadSavedToeflPositionToLive() {
    ...
    try {
        ...
        if (el && cont) {
            setTimeout(function() {
                el.style.left = ((pos.x/100)*cont.offsetWidth)+'px';
                el.style.top = ((pos.y/100)*cont.offsetHeight)+'px';
        ...
        var sz = document.getElementById('toeflQrSize'); if(sz && pos.size) sz.value = pos.size;
    } catch(e) {}
}
...
function resetToeflPosition() {
    ...
    var si = document.getElementById('toeflShowId'); if(si) si.checked = true;
...
function resizeQr(type) {
    ...
    var cvs = document.getElementById(type+'QrCanvas');
    if (cvs) {
        var id = type==='trans'?'SIEC-TR-2024-0001':'SIEC-TF-2024-0001';
        generateQrWithLogo(type+'QrCanvas', window.location.origin+'/verify.html?id='+id, size);
    }
}
...
function updateSmallPreview(type) {
    var x = document.getElementById(type+'QrX');
    var y = document.getElementById(type+'QrY');
    if (!x || !y) return;
    var xv = x.value||80, yv = y.value||85;
    var xve = document.getElementById(type+'QrXVal'); if(xve) xve.textContent = xv+'%';
    var yve = document.getElementById(type+'QrYVal'); if(yve) yve.textContent = yv+'%';
    var ov = document.getElementById(type+'SmallQr');
    if(ov) { ov.style.left = xv+'%'; ov.style.top = yv+'%'; }
    var sample = window.location.origin + '/verify.html?id=SIEC-SAMPLE';
    generateQrWithLogo(type+'SmallQrCanvas', sample, 50);
}
...
function showTranslationPrint(doc) {
    ...
    content.innerHTML =
        ...
        '<div style="position:absolute;left:'+pos.x+'%;top:'+pos.y+'%;transform:translate(-50%,-50%);background:white;padding:6px;border-radius:6px;box-shadow:0 2px 8px rgba(0,0,0,0.2);text-align:center;z-index:999;">' +
    ...
    content.innerHTML =
        ...
        (cert.file_url
            ...
            : '<div style="padding:30px;text-align:center;">' +
              ...
              '<div style="color:#666;margin:16px 0;font-size:0.9rem;">This certifies that</div>' +
              ...
              '<div style="padding:14px 20px;background:#f1f5f9;border-radius:10px;"><div style="font-size:0.7rem;color:#666;">LISTENING</div><div style="font-size:1.8rem;font-weight:800;color:#2563eb;">'+cert.listening_score+'</div></div>' +
              '<div style="padding:14px 20px;background:#f1f5f9;border-radius:10px;"><div style="font-size:0.7rem;color:#666;">STRUCTURE</div><div style="font-size:1.8rem;font-weight:800;color:#2563eb;">'+cert.structure_score+'</div></div>' +
              '<div style="padding:14px 20px;background:#f1f5f9;border-radius:10px;"><div style="font-size:0.7rem;color:#666;">READING</div><div style="font-size:1.8rem;font-weight:800;color:#2563eb;">'+cert.reading_score+'</div></div>' +
              '</div>' +
...
async function saveArticle() {
    ...
    var data = {
        title: title, slug: generateSlug(title)+'-'+Date.now(),
        content: document.getElementById('articleContent').value,
        excerpt: document.getElementById('articleExcerpt').value,
        cover_image: document.getElementById('articleCover').value,
        layout_type: le ? le.value : 'standard',
        category: document.getElementById('articleCategory').value,
        is_published: pub, published_at: pub?new Date().toISOString():null,
        updated_at: new Date().toISOString()
    };
    try {
        var r = id ? await db.from('articles').update(data).eq('id',id) : await db.from('articles').insert(data);
        if(r.error) throw r.error;
...
async function loadAdminArticles() {
    ...
    try {
        ...
        tb.innerHTML = r.data.map(function(a){
            return '<tr><td><strong>'+a.title+'</strong></td><td>'+a.category+'</td><td>'+a.layout_type+'</td>'+
...
function showProgramForm(p) {
    ...
    if (p) {
        ...
        var r=document.querySelector('input[name="programLayout"][value="'+p.layout_type+'"]'); if(r) r.checked=true;
    } else {
        document.getElementById('programFormTitle').textContent='Tambah Program';
        ['programId','programTitle','programDuration','programSchedule','programPrice','programCover','programDesc','programContent','programFeatures'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
        document.getElementById('programActive').checked=true;
        var def=document.querySelector('input[name="programLayout"][value="card"]'); if(def) def.checked=true;
    }
}
...
async function saveProgram() {
    ...
    var le=document.querySelector('input[name="programLayout"]:checked');
    var fs=document.getElementById('programFeatures').value;
    var features=fs?fs.split(',').map(function(f){return f.trim();}).filter(function(f){return f;}):[]; 
    var data={
        title:title, slug:generateSlug(title)+'-'+Date.now(),
        ...
        layout_type:le?le.value:'card', features:features,
        is_active:document.getElementById('programActive').checked,
        updated_at:new Date().toISOString()
    };
    try {
        var r=id?await db.from('learning_programs').update(data).eq('id',id):await db.from('learning_programs').insert(data);
        if(r.error) throw r.error;
        showNotification(id?'Diperbarui!':'Ditambahkan!');
...
async function loadAdminPrograms() {
    ...
    try {
        ...
        tb.innerHTML=r.data.map(function(p){
            return '<tr><td><strong>'+p.title+'</strong></td><td>'+p.program_type+'</td><td>'+p.layout_type+'</td>'+
                '<td>'+(p.price||'-')+'</td>'+
...
function showClientForm(c) {
    ...
    if(c){
        ...
        ['clientId','clientName','clientPhone','clientEmail','clientNotes'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
    }
}
...
async function loadAdminClients(){
    ...
    try {
        ...
        if(!r.data||!r.data.length){tb.innerHTML='<tr><td colspan="7" class="loading-cell">Belum ada</td></tr>';return;}
        tb.innerHTML=r.data.map(function(c){
            return '<tr><td><strong>'+c.client_name+'</strong></td><td>'+c.client_phone+'</td>'+
                ...
                '<td><div class="action-buttons">'+
                '<button class="btn btn-sm btn-primary" onclick=\'showClientForm('+JSON.stringify(c)+')\'>Edit</button>'+
                '<button class="btn btn-sm btn-danger" onclick="deleteClient(\''+c.id+'\')">Hapus</button>'+
                '</div></td></tr>';
        }).join('');
    } catch(e){console.error(e);}
}
...
async function exportClients(){
    ...
    var rows=r.data.map(function(c){return[c.client_name,c.client_phone,c.client_email||'',c.document_type,c.source_language,c.target_language,c.status,c.notes||'',formatDate(c.created_at)];});
    var csv=[h].concat(rows).map(function(r){return r.join(',');}).join('\n');
    var blob=new Blob([csv],{type:'text/csv'});
    var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='klien-siec-'+Date.now()+'.csv'; a.click();
...
async function saveTranslation(){
    ...
    var fu='', fn='';
    if(transFileData){
        try{ showNotification('Mengupload...','info'); var up=await uploadFileToSupabase(transFileData,'translations'); fu=up.url; fn=up.name; }
        catch(err){showNotification('Gagal upload: '+err.message,'error');return;}
    }
    var data={
        document_id:docId, client_name:cn, document_title:dt,
    ...
    try {
        ...
        if(r.error) throw r.error;
        showNotification('Disimpan! ID: '+docId);
        hideTranslationForm(); loadAdminTranslations(); loadDashboardStats(); showTranslationPrint(data);
...
async function loadAdminTranslations(){
    ...
    try {
        var r=await db.from('translation_documents').select('*').order('created_at',{ascending:false});
        if(!r.data||!r.data.length){tb.innerHTML='<tr><td colspan="7" class="loading-cell">Belum ada</td></tr>';return;}
        tb.innerHTML=r.data.map(function(d){
            ...
            return '<tr>'+
                ...
                '<td><div class="action-buttons">'+
                '<button class="btn btn-sm btn-success" onclick=\'showTranslationPrint('+JSON.stringify(d)+')\'>Cetak</button>'+
                db2+
                '<button class="btn btn-sm btn-danger" onclick="deleteTranslation(\''+d.id+'\')">Hapus</button>'+
                '</div></td></tr>';
        }).join('');
    } catch(e){console.error(e);}
}
...
function showStatusForm(s){
    ...
    if(s){
        ...
        ['statusId','statusClientName','statusClientPhone','statusDocType','statusDesc','statusEstimate'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
...
async function saveStatus(){
    ...
    try {
        var r;
        if(id){ r=await db.from('translation_status').update(data).eq('id',id); }
        else { data.tracking_code=generateTrackingCode(); r=await db.from('translation_status').insert(data); showNotification('Kode: '+data.tracking_code); }
        if(r.error) throw r.error;
...
async function loadAdminStatus(){
    ...
    try {
        var r=await db.from('translation_status').select('*').order('created_at',{ascending:false});
        if(!r.data||!r.data.length){tb.innerHTML='<tr><td colspan="6" class="loading-cell">Belum ada</td></tr>';return;}
        tb.innerHTML=r.data.map(function(s){
            return '<tr>'+
                ...
                '<td><div class="action-buttons">'+
                '<button class="btn btn-sm btn-primary" onclick=\'showStatusForm('+JSON.stringify(s)+')\'>Edit</button>'+
                '<button class="btn btn-sm btn-danger" onclick="deleteStatus(\''+s.id+'\')">Hapus</button>'+
                '</div></td></tr>';
        }).join('');
    } catch(e){console.error(e);}
}
...
function showToeflForm(c){
    ...
    if(c){
        ...
        ['toeflId','toeflName','toeflTestDate','toeflEmail','toeflPhone','toeflListening','toeflStructure','toeflReading','toeflTotal','toeflNotes'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
...
async function saveToefl(){
    ...
    var fu='',fn='';
    if(toeflFileData){
        try{ showNotification('Mengupload...','info'); var up=await uploadFileToSupabase(toeflFileData,'certificates'); fu=up.url; fn=up.name; }
        catch(err){showNotification('Gagal upload: '+err.message,'error');return;}
    }
    ...
    try {
        ...
        else { data.certificate_id=certId; data.barcode_data=verifyUrl; r=await db.from('toefl_certificates').insert(data); if(r.error)throw r.error; showNotification('Disimpan! ID: '+certId); showCertPrint(data); }
    ...
    try {
        ...
        tb.innerHTML=r.data.map(function(c){
            ...
            return '<tr>'+
                ...
                '<td><strong style="color:var(--primary);font-size:1.1rem;">'+c.total_score+'</strong></td>'+
                '<td>'+fb+'</td>'+
                '<td><div class="action-buttons">'+
                '<button class="btn btn-sm btn-success" onclick=\'showCertPrint('+JSON.stringify(c)+')\'>Preview</button>'+
                db2+
